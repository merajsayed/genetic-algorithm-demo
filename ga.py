...
class Individual(object):
    ...
    def create_gnome(cls):
        ...
        return [cls.mutated_genes() for _ in range(gnome_length)]
...
'''
if len(sys.argv) != 2:
    print('Usage: python ga.py "String to search"')
    sys.exit(1)
else:
    TARGET = sys.argv[1]
'''
if len(sys.argv) == 2:
    if sys.argv[0].strip().endswith(".py"):
        if sys.argv[1].strip():
    ...
    print(f"Running GA search for default target string {TARGET}")
